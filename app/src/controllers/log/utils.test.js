...
describe('log/utils', () => {
  ...
  describe('getPreviousItem', () => {
    test('should return null in case of item array length < 2', () => {
      ...
      expect(getPreviousItem(1, [{ id: 1 }])).toBe(null);
    });
    test('should return null if the first item is active', () => {
      expect(getPreviousItem(0, itemsArray)).toBe(null);
    });
    test('should return item if active item index > 0', () => {
      expect(getPreviousItem(1, itemsArray)).toEqual({ id: 0 });
      expect(getPreviousItem(2, itemsArray)).toEqual({ id: 1 });
    });
  });
  ...
  describe('getNextItem', () => {
    ...
    test('should return null if the last item is active', () => {
      expect(getNextItem(2, itemsArray)).toBe(null);
    });
    test('should return item if active item is not last', () => {
      expect(getNextItem(0, itemsArray)).toEqual({ id: 1 });
      expect(getNextItem(1, itemsArray)).toEqual({ id: 2 });
    });
  });

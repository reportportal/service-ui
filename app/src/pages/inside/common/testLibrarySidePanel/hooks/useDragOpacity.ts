...
const isFolderInDraggedSubtrees = (folderId: number, folderItem: FolderDragItem) =>
  folderItem.folders.some((draggedFolder) => isInFolderSubtree(folderId, draggedFolder));

const useDragLayerState = (): DragLayerState =>
...
export const useFolderDragOpacity = (folderId: number, checkboxState: CheckboxSelectionState) => {
  const { isDragging, itemType, item } = useDragLayerState();
  ...
  if (itemType === FOLDER_DRAG_TYPE) {
    ...
    return isFolderInDraggedSubtrees(folderId, folderItem);
  }

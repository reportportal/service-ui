...
interface TestPlanDropZonesProps {
  isAddToTestPlanLoading?: boolean;
  isAddToLaunchLoading?: boolean;
  onAddTestCasesToTestPlan: (testCases: TestCase[]) => void;
  onAddTestCasesAndCreateLaunch: (testCases: TestCase[]) => void;
  onAddFolderToTestPlan: (folders: TransformedFolder[]) => void;
  onAddFolderAndCreateLaunch: (folders: TransformedFolder[]) => void;
}

export const TestPlanDropZones = ({
  isAddToTestPlanLoading = false,
  isAddToLaunchLoading = false,
  ...
  const isBusy = isAddToTestPlanLoading || isAddToLaunchLoading;

  if (!isDraggingItems && !isBusy) {
    return null;
  }

  const handleAddDrop = (item: DragItem, droppedItemType: string | symbol | null) => {
    if (droppedItemType === FOLDER_DRAG_TYPE) {
      onAddFolderToTestPlan((item as FolderDragItem).folders);
    } else {
  ...
  const handleAddAndCreateLaunchDrop = (
    ...
    if (droppedItemType === FOLDER_DRAG_TYPE) {
      onAddFolderAndCreateLaunch((item as FolderDragItem).folders);
    } else {
  ...
  return createPortal(
    <div className={cx('test-plan-drop-zones')}>
      <DropZone
        ...
        isLoading={isAddToLaunchLoading}
        isDisabled={isBusy}
      ...
      <DropZone
        ...
        isLoading={isAddToTestPlanLoading}
        isDisabled={isBusy}
        showIcon

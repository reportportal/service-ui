...
import { TestCasesResponse } from './fetchAllTestCases';
...
export const fetchAllTestPlanTestCases = async (
  projectKey: string,
  testPlanId: number,
  params: Record<string, string | number> = DEFAULT_PAGE_PARAMS,
  fetchedTestCases: TestCase[] = [],
): Promise<TestCase[]> => {
  const response = await fetch<TestCasesResponse>(

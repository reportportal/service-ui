...
export class DashboardGridItem extends Component {
  ...
  render() {
    const { item, currentUser, intl } = this.props;
    const {
      name,
      description,
      owner,
    ...
    return (
      <NavLink to={`dashboard/${id}`} className={cx('grid-view')}>
        <div className={cx('grid-view__inner')}>
          <div className={cx('grid-cell', 'name')}>
            <h3 className={cx('dashboard-link')}>
              {name}
            </h3>
          </div>
          <div
            className={cx('grid-cell', 'description', 'preview', `preview-${DashboardGridItem.calculateGridPreviewBaseOnWidgetId(id)}`)}
          >
            <p>
              {description}
            </p>
          </div>
          <div className={cx('grid-cell', 'owner')}>
            {owner}
          </div>
          <div className={cx('grid-cell', 'shared')}>
            {share && <Fragment><Icon type="icon-tables" />
              <span className={cx('shared-text')}>
                {intl.formatMessage(messages.dashboardIsShared)}
              </span>
            </Fragment>}
            {currentUser !== owner &&
            (<Fragment><Icon type="icon-planet" />
              <span className={cx('shared-text')}>

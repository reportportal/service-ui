...
const localConfig = (trackEvent: (data: Partial<object>) => void) => ({
  name: 'Local',
  eventInfo: {
...
const remoteConfig = (trackEvent: (data: Partial<object>) => void) => ({
  name: 'Remote',
  eventInfo: {
  ...
  content: (
    <div className={cx('content-wrapper')}>
      ...
      <p className={cx('paragraph')}>
        The remote server must be deployed and running in HTTP mode (MCP_MODE=http).
      </p>
      <p className={cx('paragraph')}>
        Choose the appropriate{' '}
        <a

...
export type CreateMilestoneFormValues = CreateMilestonePayload;
...
export const useCreateMilestone = () => {
  ...
  const submitMilestone = async (payload: CreateMilestoneFormValues) => {
    try {
      showSpinner();

      await fetch<TmsMilestoneRS>(URLS.tmsMilestone(projectKey), {
        method: 'post',
        data: {
          name: payload.name,
          type: payload.type,
      ...
      dispatch(
        showSuccessNotification({
          messageId: 'milestoneCreatedSuccess',
        }),
      );

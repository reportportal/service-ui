        ...
        var on_resize_handler = function () {
            if (self._is_one_column_mode()) {
                ...
                _.each(self.grid.nodes, function (node) {
                    ...
                    if (!node.no_move) {
                        // Custom change: node.el.draggable('disable'); 
                        node.el.draggable({ disabled: true });
                    }
                    if (!node.no_resize) {
                        // Custom change: node.el.resizable('disable');
                        node.el.resizable({ disabled: true });
                    }
                });
            }
            else {
    ...
    GridStack.prototype._prepare_element = function (el) {
        ...
        var on_end_moving = function (event, ui) {
            ...
            var widget = _.find(self.widgets, function(w){
                var id = w.id.slice(7, w.id.length);
                return id === node.el.attr('id');
            });
            if(widget){
                if(widget.charts){
                    _.each(widget.charts, function(c){
                        if(_.isFunction(c)){
                            c.update();
                        }
                    })
                }
                else if(widget.chart && _.isFunction(widget.chart)){
                    widget.chart.update();
                }
            }                
        ...
        el.draggable(_.extend(this.opts.draggable, {
            ...
            containment: this.opts.is_nested ? this.container.parent() : this.opts.draggable.containment
        ...
        if (node.no_move || this._is_one_column_mode()) {
            // Custom change: el.draggable('disable'); 
            el.draggable({ disabled: true });
        }

        if (node.no_resize || this._is_one_column_mode()) {            
            // Custom change: el.resizable('disable');

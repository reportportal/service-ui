...
nv.interactiveGuideline = function() {
    ...
    var isMSIE = "ActiveXObject" in window
        , isAtLeastIE11 = !!(window.navigator.userAgent.match(/Trident/) && !window.navigator.userAgent.match(/MSIE/));
...
(function() {
    ...
    nv.tooltip.show = function(pos, content, gravity, dist, parentContainer, classes) {
        ...
        nv.tooltip.calcTooltipPosition(pos, gravity, dist, container, parentContainer); //custom
    };
    ...
    nv.tooltip.calcTooltipPosition = function(pos, gravity, dist, container, parent) {
        ...
        switch (gravity) {
            ...
            case 's':
                ...
                if (tLeft < scrollLeft) left = scrollLeft + 5;
                if (tLeft + width > windowWidth) left = left - width/2 + 5;
                if (scrollTop > tTop) top = scrollTop - $(parent).offset().top;
                break;
            case 'none':
                left = pos[0];
                top = pos[1] - dist;
        ...
        selection.each(function(data) {
            ...
            switch (axis.orient()) {
                ...
                case 'bottom':
                    ...
                    var w;
                    if (scale.range().length < 2) {
                        w = 2*scale.range()[0] + chart.rangeBand();
                    } else if (scale.range().length === 2) {
                        w = scale.range()[0] == 0 ? scale.range()[1] : scale.range()[scale.range().length-1]+(scale.range()[1]-scale.range()[0]);
                    } else {
                        w = scale.range()[scale.range().length-1]+(scale.range()[1]-scale.range()[0]);
                    }
                    axisLabel
...
nv.models.trendBar = function() {
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            //console.log(yDomain, ', ', d3.extent(d3.merge(seriesData).map(function(d) { return d.y }).concat(forceY)));
            //console.log(forceY);
            x   .domain(xDomain || d3.merge(seriesData).map(function(d) { return d.x }))
                .rangeBands(xRange || [0, availableWidth], .1);
            y   .domain(yDomain || d3.extent(d3.merge(seriesData).map(function(d) { return d.y }).concat(forceY)));
            ...
            if (showValues) {
                ...
                bars.select('text')
                    ...
                    .attr('y', function(d,i) { return getY(d,i) < 0 ? y(getY(d,i)) - y(0) + 12 : -4 })
                ;
                bars.select('text').filter(function (d, i) {
                    return i % Math.ceil(data[0].values.length/2 / (availableWidth / 100)) !== 0;
                }).style('opacity', 0)
                ;
            ...
            bars.watchTransition(renderWatch, 'discreteBar: bars')
                .attr('transform', function(d,i) {
                    var left = x(getX(d,i)) + x.rangeBand() * .05,
                        top = i == 0 ? y(d.value) : getY(d,i) < 0 ? y(prev) : y(prev + getY(d,i));

                    prev += i == 0 ? d.value : getY(d,i);
                    return 'translate(' + left + ', ' + (d.y == 0 ? top-1 : top) + ')'
                })
                .select('rect')
                .attr('height', function(d,i) {
                    return (i!==0) ? y(0) - y((getY(d,i) < 0) ? Math.abs(getY(d,i)) + yDomain[0] : getY(d,i) + yDomain[0]) || 1.5 : 1.5;
                });
...
nv.models.trendBarChart = function() {
    "use strict";
    ...
    var trendbar = nv.models.trendBar()
        , xAxis = nv.models.axis()
        , yAxis = nv.models.axis()
        ;
    ...
    var showTooltip = function(e, offsetElement) {
        var left = e.pos[0] + ( offsetElement.offsetLeft || 0 ),
            top = (e.value < 0) ? e.pos[1]/2 : e.pos[1] + ( offsetElement.offsetTop || 0),
            x = xAxis.tickFormat()(trendbar.x()(e.point, e.pointIndex)),
            y = yAxis.tickFormat()(trendbar.y()(e.point, e.pointIndex)),
            content = tooltip(e.series.key, x, y, e, chart);
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            barsWrap.transition().call(trendbar);


            defsEnter.append('clipPath')
                .attr('id', 'nv-x-label-clip-' + trendbar.id())
                .append('rect');

            g.select('#nv-x-label-clip-' + trendbar.id() + ' rect')
                .attr('width', x.rangeBand() * (staggerLabels ? 2 : 1))
                .attr('height', 16)
                .attr('x', -x.rangeBand() / (staggerLabels ? 1 : 2 ));
            ...
            if (showXAxis) {
                ...
                g.select('.nv-x.nv-axis')
                    .attr('transform', 'translate(0,' + (y.range()[0] + ((trendbar.showValues() && y.domain()[0] < 0) ? 16 : 0)) + ')');
                g.select('.nv-x.nv-axis').call(xAxis);
        ...
        renderWatch.renderEnd('trendBar chart immediate');
        return chart;
    }
    ...
    trendbar.dispatch.on('elementMouseover.tooltip', function(e) {
        e.pos = [e.pos[0] +  margin.left, e.pos[1] + margin.top];
        dispatch.tooltipShow(e);
    });

    trendbar.dispatch.on('elementMouseout.tooltip', function(e) {
        dispatch.tooltipHide(e);
    });
    ...
    chart._options = Object.create({}, {
        ...
        duration: {get: function(){return duration;}, set: function(_){
            duration = _;
            renderWatch.reset(duration);
            trendbar.duration(duration);
            xAxis.duration(duration);
            yAxis.duration(duration);
        }},
        color:  {get: function(){return color;}, set: function(_){
            color = nv.utils.getColor(_);
            trendbar.color(color);
        }},
        rightAlignYAxis: {get: function(){return rightAlignYAxis;}, set: function(_){
            rightAlignYAxis = _;
            yAxis.orient( (_) ? 'right' : 'left');
    ...
    nv.utils.inheritOptions(chart, trendbar);
    nv.utils.initOptions(chart);
            ...
            if (align) {
                ...
                series.each(function(d,i) {
                    ...
                    try {
                        ...
                        nodeTextLength = nv.utils.calcApproxTextWidth(legendText) || 0; // custom
                    }
        ...
        selection.each(function(data) {
            ...
            if (showXAxis) {
                ...
                if (reduceXTicks) {
                    xTicks
                        .filter(function (d, i) {
                            return i % Math.ceil(data[0].values.length/4 / (availableHeight / 100)) !== 0;
                        })
                        .selectAll('text, line')
                        .style('opacity', 0);
                }
                    ...
                    if (labelSunbeamLayout) {
                        ...
                        if(d.value){
                            var hashKey = createHashKey(center),
                                keys = d3.keys(labelLocationHash);
                            keys.forEach(function(k){
                                var a = k.split(','),
                                    ...
                                    x2 = parseInt(b[0]),
                                    y2 = parseInt(b[1]);
                                if(!(x1 + avgWidth < x2 || y1 + avgHeight < y2 || x1 > x2 + avgWidth || y1 > y2 + avgHeight)){
                                    center[1] += avgHeight;
                                }
                            });
            ...
            if (showLegend) {
                if (legendPosition === "top") {
                    ...
                    wrap.select('.nv-legendWrap')
                        .datum(data)
                        .call(legend)
                        .attr('transform', 'translate(' + (availableWidth) +',' + (-(margin.top-10)) +')');
                }
            }
            wrap.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
        ...
        selection.each(function(data) {
            ...
            path.enter().append('path').attr('class', function(d,i) { return 'nv-area nv-area-' + i })
                ...
                .on('click', function(d,i) {
                    var d3mouse = d3.mouse(this);
                    var mouseX = d3mouse[0];
                    var xValue = chart.xScale().invert(mouseX);
                    var pointIndex = nv.interactiveBisect(d.values, xValue, chart.x());
        ...
        selection.each(function(data) {
            ...
            interactiveLayer.dispatch.on('elementMousemove', function(e) {
                ...
                var valueFormatter = (stacked.style() == 'expand') ?
                    function(d,i) {
                        return d>=1 ? (d%1===0 ? d3.format(".0%")(d) : d3.format(".2%")(d)) : (d*100%1===0 ? d3.format(".0%")(d) : d3.format(".2%")(d));
                    } : //custom

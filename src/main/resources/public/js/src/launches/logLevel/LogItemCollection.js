...
define(function (require) {
    ...
    var LogItemCollection = Backbone.Collection.extend({
        ...
        getParamsForUrl: function () {
            ...
            filterOptions = _.map(this.filterModel.getOptions(), function (option) {
                return 'log.' + option;
            });
        ...
        findLogPage: function (logId, cleanFilters) {
            ...
            if (cleanFilters) {
                _.each(params, function (param) {
                    if (~param.search(/(FATAL)/)) {
                        newParams.push('filter.gte.level=ERROR');
                    } else if (!~param.search(/(binary_content|cnt\.message)/)) {
